...
export async function POST(request) {
  try {
    ...
    const result = await fetchLinkedInCertifications({
      text,
      linkedinUrl: linkedinUrl || url || user.linkedinUrl || "",
      title,
      issuer,
      credentialId,
      verificationUrl,
    ...
    console.error("LinkedIn certifications POST error:", err);
    return NextResponse.json({ error: "Failed to extract certifications: " + err.message }, { status: 500 });
  }
}
