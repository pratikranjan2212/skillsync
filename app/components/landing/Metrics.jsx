...
export default function Metrics() {
  ...
  return (
    <motion.section
      ...
      className="py-14 sm:py-16 md:py-24 px-3.5 sm:px-6 2xl:px-8 max-w-7xl 2xl:max-w-[1536px] mx-auto"
    >
      {/* Outer Card Wrapper */}
      <div
        onMouseEnter={() => setIsAutoPlaying(false)}
        onMouseLeave={() => setIsAutoPlaying(true)}
        className="relative bg-white rounded-3xl sm:rounded-[44px] border border-black/10 p-5 sm:p-10 md:p-12 shadow-2xl overflow-hidden group/card"
      >
        {/* Top Header Row */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 mb-6 sm:mb-8 pb-5 sm:pb-6 border-b border-black/8">
          <div>
            <div className="inline-flex items-center gap-2 sm:gap-2.5 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-2.5 sm:mb-3 shadow-xs">
        ...
        <div className="relative min-h-[340px] sm:min-h-95 flex items-center justify-center py-2">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="lg:col-span-5 flex flex-col justify-between h-full space-y-3 sm:space-y-4">
                <div>
                  <div className="flex items-center gap-2 sm:gap-2.5 mb-2.5 sm:mb-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-600 shadow-xs">
                      <Icon className="w-5 h-5 sm:w-5.5 sm:h-5.5" />
                    </div>
                    <span className="text-[10px] sm:text-xs font-black uppercase tracking-wider text-emerald-700 bg-emerald-100/80 px-2.5 sm:px-3 py-1 rounded-full border border-emerald-300 shadow-2xs">
                      {activeMetric.badgeText}
                    </span>
                  </div>

                  <span className="text-[11px] sm:text-xs font-extrabold uppercase tracking-wider text-[#494D4D] block mb-1">
                    {activeMetric.label}
                  </span>
                  ...
                  <motion.div
                    ...
                    className="text-4xl sm:text-6xl md:text-7xl font-black text-[#111111] tracking-tight leading-none my-2"
                  >
                    {activeMetric.value}
                  </motion.div>

                  <p className="text-xs sm:text-sm md:text-base text-[#494D4D] font-medium leading-relaxed mt-2 sm:mt-3">
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-extrabold shadow-md hover:shadow-lg transition-all hover:scale-105 active:scale-95 cursor-pointer"
                  >
              ...
              <div className="lg:col-span-7 bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-black/8 shadow-md flex flex-col justify-between space-y-4 sm:space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-2.5">
                      {["Gender 🚫", "College Tier 🚫", "Name 🚫", "Photo 🚫"].map((item, i) => (
                        <motion.div
                          ...
                          className="p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-neutral-50 border border-neutral-200 text-center font-extrabold text-[11px] sm:text-xs text-neutral-800 flex items-center justify-center gap-1 shadow-2xs hover:bg-neutral-100 transition-colors"
                        >
                          {item}
                        </motion.div>
                      ))}
                    </div>
                    <div className="p-3 sm:p-3.5 rounded-2xl bg-emerald-950 text-emerald-300 text-xs font-bold flex items-center gap-2">
                      <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                      <span>Candidate ranking vectors evaluate strictly verified skills and evidence citations.</span>
                    </div>
                  </div>
                )}
                ...
                {activeMetric.id === "taxonomy" && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="p-3.5 sm:p-4 rounded-2xl bg-neutral-50 border border-neutral-200 flex items-center gap-3">
                        <span className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-emerald-500 animate-pulse shrink-0"></span>
                        <div>
                          <div className="font-extrabold text-xs text-neutral-900">ESCO European Taxonomy</div>
                          <div className="text-[10px] sm:text-[11px] text-neutral-500 font-medium">Standardized Skill Classification</div>
                        </div>
                      </div>
                      <div className="p-3.5 sm:p-4 rounded-2xl bg-neutral-50 border border-neutral-200 flex items-center gap-3">
                        <span className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-sky-500 animate-pulse shrink-0"></span>
                        <div>
                          <div className="font-extrabold text-xs text-neutral-900">US O*NET Framework</div>
                          <div className="text-[10px] sm:text-[11px] text-neutral-500 font-medium">Occupational Competency Matrix</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeMetric.id === "accuracy" && (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs sm:text-sm font-black text-emerald-600">99.4% Benchmark</span>
                    </div>
                    ...
                    <div className="p-3 sm:p-3.5 rounded-2xl bg-neutral-900 text-white text-xs font-bold flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                      <span>Zero Blind Scoring • 100% Citation Grounded</span>
                      <span className="px-2 py-0.5 rounded bg-emerald-500 text-black font-extrabold text-[10px] self-start sm:self-auto">Verified</span>
                    </div>
                  </div>
                )}
                ...
                <div className="pt-2 flex items-center gap-1.5 sm:gap-2 flex-wrap">
                  {activeMetric.metricsList.map((item, i) => (
                    <span key={i} className="px-2.5 sm:px-3 py-1 rounded-xl bg-neutral-100 text-neutral-800 text-[11px] sm:text-xs font-bold border border-neutral-200/80 shadow-2xs">
                      ✓ {item}
                    </span>
                  ))}
                </div>
      ...
      <AnimatePresence>
        {activeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3.5 sm:p-6 bg-black/60 backdrop-blur-md">
            ...
            <motion.div
              ...
              className="relative z-10 w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-black/10 p-5 sm:p-8 overflow-hidden text-neutral-900 max-h-[88vh] overflow-y-auto"
            >
              <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 sm:top-5 sm:right-5 p-2 rounded-full bg-neutral-100 hover:bg-neutral-200 text-neutral-600 transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
              ...
              {activeModal === "speed" && (
                <div className="space-y-5 sm:space-y-6">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 sm:p-3 rounded-2xl bg-emerald-500 text-black">
                      <Zap className="w-5 h-5 sm:w-6 sm:h-6" />
                    </div>
                    <div>
                      <span className="text-xs font-black uppercase tracking-wider text-emerald-600">Speed Benchmark</span>
                      <h3 className="text-xl sm:text-2xl font-black text-neutral-900">Sub-400ms Verification Latency</h3>
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-emerald-950 text-white flex items-center justify-between shadow-lg">
                    ...
                    <div className="text-xl sm:text-2xl font-black text-emerald-400">380ms</div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-5 sm:space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="text-xl sm:text-2xl font-black text-neutral-900">100% Demographic Bias Elimination</h3>
                    </div>
                  </div>
                  <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed font-medium">
                    SkillSync explicitly masks non-skill demographic attributes to guarantee 100% meritocratic matching.
                  </p>
                  <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-neutral-900 text-white border border-neutral-800 space-y-3">
                    <div className="text-xs text-emerald-400 font-extrabold">Excluded from Ranking Vector:</div>
                    <div className="text-xs sm:text-sm font-semibold text-neutral-300">[Gender, College Tier, Name, Photo]</div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-5 sm:space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="text-xl sm:text-2xl font-black text-neutral-900">12,500+ Normalized Skill Vectors</h3>
                    </div>
                  </div>
                  <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed font-medium">
                ...
                <div className="space-y-5 sm:space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="text-xl sm:text-2xl font-black text-neutral-900">99.4% Recommendation Accuracy</h3>
                    </div>
                  </div>
                  <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed font-medium">
