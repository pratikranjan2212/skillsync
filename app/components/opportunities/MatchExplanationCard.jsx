    ...
    <div className="bg-white rounded-4xl p-6 sm:p-8 shadow-xl border border-black/5 flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pb-6 border-b border-neutral-100">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <div className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-base sm:text-lg font-black border shadow-xs ${band.badgeBg}`}>
      ...
      {subScores && Object.keys(subScores).length > 0 && (
        <div>
          <h3 className="text-base font-bold text-[#111111] flex items-center gap-2 mb-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {evidenceList.length > 0 ? (
            ...
            <p className="text-sm text-neutral-500 italic">No direct supporting evidence matched for this opportunity.</p>
          )}
        </div>
      </div>
      ...
      {relatedList.length > 0 && (
        <div>
          <h3 className="text-lg font-bold text-[#111111] flex items-center gap-2 mb-3">
          ...
          <div className="grid grid-cols-1 gap-3">
            {relatedList.map((item, index) => (
              <div
                key={index}
                className="bg-teal-50/50 p-4 rounded-2xl border border-teal-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-[#111111]">{item.skill}</span>
                      <span className="text-[10px] bg-teal-200/70 text-teal-900 font-bold px-1.5 py-0.5 rounded">
                        via {item.sourceSkill}
                      </span>
                    </div>
                    <div className="text-xs text-teal-900/80 mt-0.5">{item.reason}</div>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {missingReqList.map((skillItem, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="flex flex-col gap-1 px-3.5 py-2.5 bg-amber-50/70 text-amber-900 border border-amber-200/80 rounded-xl text-xs font-semibold flex-1 min-w-[240px]"
                      >
                        <div className="flex items-center gap-2">
      ...
      <div className="p-4 rounded-2xl bg-neutral-50 border border-black/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-neutral-600">
