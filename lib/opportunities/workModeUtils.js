...
export function resolveApplicationUrl(companyName = "", title = "", source = "Indeed") {
  ...
  if (source === "Indeed") {
    return `https://in.indeed.com/jobs?q=${encodeURIComponent(`${comp} ${role}`.trim())}&l=India`;
  }
  return `https://www.linkedin.com/jobs/search/?keywords=${encodeURIComponent(`${comp} ${role}`.trim())}`;
}
