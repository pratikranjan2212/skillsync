...
/**
 * Returns a nodemailer transporter using Gmail SMTP with an app password.
 * Evaluates env dynamically or on invocation so process.env / env.js values are always fresh.
 */
function getTransporter() {
  const user = (env.emailFrom || process.env.EMAIL_FROM || "skillsyncauto@gmail.com").trim();
  const pass = (env.emailAppPassword || process.env.EMAIL_APP_PASSWORD || "auem kvdy rsyz uaqz").replace(/\s+/g, "");
...
export async function sendOtpEmail(to, otp, name = "Student") {
  ...
  const user = env.emailFrom || process.env.EMAIL_FROM || "skillsyncauto@gmail.com";
  const transporter = getTransporter();

  const mailOptions = {
    from: `"SkillSync" <${user.trim()}>`,
    to,
