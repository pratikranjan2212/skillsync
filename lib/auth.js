  ...
  providers: [
    ...
    LinkedIn({
      ...
      profile(profile) {
        ...
        const slug = (resolvedName || profile.given_name || "").toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
        ...
        return {
          ...
          email: profile.email,
          image: photo || null,
          linkedinUrl: slug ? `https://linkedin.com/in/${slug}` : undefined,
          role: "student",
          emailVerified: profile.email_verified ? new Date() : null,
        };
      },
        ...
        try {
          ...
          if (existing) {
            ...
            if (account.provider === "linkedin") {
              ...
              let linkedinUrl = rawVanity ? (rawVanity.startsWith("http") ? rawVanity : `https://linkedin.com/in/${rawVanity}`) : null;
              if (!linkedinUrl && (profile?.name || user?.name || existing.name)) {
                const slug = (profile?.name || user?.name || existing.name).toLowerCase().replace(/[^a-z0-9]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
                if (slug) linkedinUrl = `https://linkedin.com/in/${slug}`;
              }
