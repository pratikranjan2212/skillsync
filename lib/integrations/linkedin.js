...
export async function scrapeLinkedInProfileCertifications(username) {
  ...
  // Strategy 2: Live LinkedIn Scraper API Gateways (Scrapingdog, RapidAPI, Scrapin)
  if (scraperKey && !scraperKey.includes("placeholder")) {
    try {
      ...
      const endpoints = [
        {
          url: `https://api.scrapingdog.com/linkedin/?api_key=${scraperKey}&type=profile&linkId=${cleanUser}`,
          headers: { Accept: "application/json" },
        },
        {
          url: `https://fresh-linkedin-profile-data.p.rapidapi.com/get-linkedin-profile?linkedin_url=${encodeURIComponent(profileUrl)}`,
          headers: {
        ...
        {
          url: `https://linkedin-data-api.p.rapidapi.com/get-profile-data-by-url?url=${encodeURIComponent(profileUrl)}`,
          headers: {
            Accept: "application/json",
            "x-rapidapi-key": scraperKey,
        ...
        {
          url: `https://api.scrapin.io/enrichment/profile?apikey=${scraperKey}&linkedInUrl=${encodeURIComponent(profileUrl)}`,
          headers: { Accept: "application/json" },
        },
      ];
      ...
      for (const target of endpoints) {
        try {
          ...
          // Continue to next scraper strategy
        }
      }
      clearTimeout(timeoutId);
    } catch (err) {
