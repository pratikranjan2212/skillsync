...
import { decodeHtml, extractCompensationAndMode, formatSalary } from "@/lib/ingestion/linkedin";
    ...
    if (res.ok) {
      ...
      if (Array.isArray(items) && items.length > 0) {
        const parsed = items.map((item, idx) => {
          ...
          // 1. Check direct structured salary from Indeed
          const directSalary = formatSalary(item.salary);

          // 2. Extract compensation and work mode from description
          ...
          // Pick most specific salary
          let salary = directSalary || extractedDesc.stipend;
          if (!salary) {
            salary = title.toLowerCase().includes("intern")
              ? "₹15,000 / month"
              : "₹4,00,000 / yr";
          }
          ...
          return {
            id: `job-indeed-${item.id || Date.now()}-${idx}`,
            externalId: `ext-indeed-${item.id || idx}`,
            title,
            company,
            location: loc,
            workMode,
