      ...
      if (Array.isArray(items) && items.length > 0) {
        const parsed = items.map((item, idx) => {
          ...
          const directIndeedUrl =
            ...
            (item.jobKey ? `https://in.indeed.com/viewjob?jk=${item.jobKey}` : null) ||
            (item.id && !String(item.id).includes("job-") && !String(item.id).includes("indeed-") ? `https://in.indeed.com/viewjob?jk=${item.id}` : null) ||
